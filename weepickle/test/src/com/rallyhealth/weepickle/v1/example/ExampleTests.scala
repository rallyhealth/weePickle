...
object ExampleTests extends TestSuite {
  ...
  val tests = Tests {
    ...
    test("keyed") {
      ...
      /*
       * Three styles to achieve mapping of keys to/from snake case:
       *  1) As described in http://www.lihaoyi.com/upickle/#CustomConfiguration. More appropriate
       *     if you want to customize something on a project-wide basis. This has issues interacting
       *     with libraries that expect Froms/Tos from WeePickle rather than a custom object
       *     extending of AttributeTagged. So in general, this is not recommended (but it is the
       *     default in upickle, so still worth describing).
       *  2) By using Visitor.mapKeys to decorate on To side of every transformation. This puts the
       *     control of what key mapping is done at the point of execution. May be more appropriate
       *     if the same object needs to have its keys mapped in different ways depending on the target
       *     (e.g., snake_case for Json, camelCase for YAML). It should interoperate fine with libraries
       *     requiring (or supplying) Froms/Tos.
       *  3) By using FromTo.bimapKeys as part of declaring the implicit FromTo (e.g., modifying default
       *     that is generated by macroFromTo). (Or, separately, To.mapKeysTo and From.comapFromKeys.)
       *     This puts the control of what key mapping is done at the point of declaration. This is more
       *     appropriate if the same object should always have its keys be mapped the same way, regardless
       *     of the target (e.g., snake_case for Json, YAML, etc). It should also interoperate fine with
       *     libraries requiring (or supplying) Froms/Tos. Note that, because the key mapping is recursive,
       *     the bimapKeys only needs to be applied to the top-level FromTo, not at any lower levels.
       */
      ...
      test("snakeCase by using Visitor.mapKeys") {
        // Default read-writing
        FromScala(Thing(1, "gg")).transform(ToJson.string) ==>
          """{"myFieldA":1,"myFieldB":"gg"}"""
      ...
      test("snakeCase by using FromTo.bimapKeys") {
        // Default read-writing
        FromScala(SnakeyThing(1, "gg")).transform(ToJson.string) ==>
          """{"my_field_a":1,"my_field_b":"gg"}"""

        FromJson("""{"my_field_a":1,"my_field_b":"gg"}""").transform(to[SnakeyThing]) ==>
          SnakeyThing(1, "gg")
      }
