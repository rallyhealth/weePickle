  ...
  val tests = TestSuite {
    test("nullAsNone") {
      ...
      test("customPickler") {
        ...
        object CustomThing2 {
          implicit val rw: OptionPickler.FromTo[CustomThing2] = /*weepickle.default*/ OptionPickler
            .fromTo[String]
            .bimap[CustomThing2](
              x => s"${x.i} ${x.s}",
              str => {
      ...
      test("Js") {
        ...
        test("option") {
          test("write") { fromScala(Some(Bar(123, "abc"))).transform(ToJson.string) ==> """["abc",123]""" }
          test("readSome") { FromJson("""["abc",123]""").transform(toScala[Option[Bar]]) ==> Some(Bar(123, "abc")) }
          test("readNull") { FromJson("""null""").transform(toScala[Option[Bar]]) ==> None }
        }
      }
