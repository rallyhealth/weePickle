...
object EnumTests extends TestSuite {
  ...
  def tests = Tests {
    ...
    test("name") {
      ...
      /*
       * Test fails in Scala 3, which does not support the .toString operation
       * on a nameless enumeration value.
       *
       *   java.lang.IllegalAccessException: Class scala.Enumeration can not access a member of class...
       *
       * (Only .id works.) Since this is kind of a corner case, we should probably just accept it as a
