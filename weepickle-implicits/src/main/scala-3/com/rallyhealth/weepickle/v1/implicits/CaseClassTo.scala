...
trait CaseClassToPiece extends MacrosCommon :
  ...
  /*
   * Default string decoding for Scala 3 enums. Mirror should always be for the sealed trait generated by the
   * enum, which is a Sum type. So any calls here with Product type mirrors are rejected as compilation errors.
   */
  inline def macroEnumTo[T](using m: Mirror.Of[T]): To[T] = inline m match {
    case m: Mirror.ProductOf[T] =>
      error("Enumeration macro used for non-sealed trait")

    case m: Mirror.SumOf[T] =>
      val valueOf: String => T = macros.enumValueOf[T]
      new SimpleTo[T] {
        override def expectedMsg: String = "expected String (enum)"
        override def visitString(cs: CharSequence): T = valueOf(cs.toString)
      }
  }
  ...
  implicit class ToExtension(r: To.type):
    inline def derived[T](using Mirror.Of[T]): To[T] = inline erasedValue[T] match
      case _: scala.reflect.Enum => macroEnumTo[T]
      case _ => macroTo[T]
  end ToExtension
