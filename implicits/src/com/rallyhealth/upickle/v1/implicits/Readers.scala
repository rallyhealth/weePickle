...
trait Readers extends com.rallyhealth.upickle.v1.core.Types with Generated with MacroImplicits{
  ...
  implicit val LongReader: Reader[Long] = new SimpleReader[Long] {
    override def expectedMsg = "expected number"
    override def visitString(d: CharSequence, index: Int) = com.rallyhealth.upickle.v1.core.Util.parseLong(d, 0, d.length())
  ...
  implicit val DurationReader = new MapStringReader( s =>
    if (s.charAt(0) == 'i' &&
      ...
      Duration.MinusInf
               ...
               s.length() == 5){
      Duration.Undefined
    }else Duration(com.rallyhealth.upickle.v1.core.Util.parseLong(s, 0, s.length()), TimeUnit.NANOSECONDS)
  )
