...
object WeeJson {
  def transform[T](r: JsonTransformable, v: com.rallyhealth.weepickle.v1.core.Visitor[_, T]): T = r.transform(v)

  /**
    * Read the given JSON input as a JSON struct
    */
  def read(s: JsonTransformable): Value.Value = transform(s, Value)
  ...
  /**
    ...
    */
  def validate(s: JsonTransformable): Unit = transform(s, NoOpVisitor)
  /**
    * Parse the given JSON input and write it to a string with
    * the configured formatting
    */
  def reformat(s: JsonTransformable, indent: Int = -1, escapeUnicode: Boolean = false): String = {
  ...
  /**
    ...
    */
  def reformatTo(s: JsonTransformable, out: java.io.Writer, indent: Int = -1, escapeUnicode: Boolean = false): Unit = {
